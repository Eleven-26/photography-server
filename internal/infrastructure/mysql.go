...
func InitMySQL(c *config.Config) error {
	dbOnce.Do(func() {
		...
		// 前提：全局 TracerProvider 已就绪 —— main.go 中 InitJaeger 必须先于 InitMySQL 执行。
		// 适用通道：仅 Jaeger/OTel 通道安装此插件；SkyWalking-go（native）通道的 SQL 埋点由
		// 编译期注入的 agent 增强 database/sql 完成，与本插件无关 —— 因此这里以 JaegerEnabled()
		// 为条件，避免 native 注入构建也误装 OTel 插件造成 SQL 双采集。
		// 已知限制（两条通道通用）：SQL span 挂在 gorm Statement.Context 下，业务调用未透传请求 ctx 时
