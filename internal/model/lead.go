...
type Lead struct {
	...
	Status       string  `gorm:"column:status;size:20;default:pending;comment:状态 pending-待回复 quoting-待报价 quoted-已报价 confirmed-已成交 lose-已流失" json:"status"`
...
type Quote struct {
	...
	Status      string  `gorm:"column:status;size:20;default:draft;comment:状态 draft-草稿 sent-已发送 accepted-已接受 rejected-已拒绝 converted-已成交" json:"status"`
	Remark      string  `gorm:"column:remark;size:500;comment:备注" json:"remark"`
	OwnerID     int64   `gorm:"column:owner_id;index;comment:负责人ID" json:"owner_id"`
	ShootDate   *string `gorm:"column:shoot_date;comment:意向拍摄日期" json:"shoot_date"`
}
