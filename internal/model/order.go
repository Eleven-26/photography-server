...
type Order struct {
	...
	Status           enum.OrderStatus   `gorm:"column:status;type:tinyint;default:1;comment:订单状态 1-待定金 2-待拍摄 3-拍摄中 4-精修中 5-待交付 6-已完成 7-已取消" json:"status"`
	PaymentStatus    enum.PaymentStatus `gorm:"column:payment_status;type:tinyint;default:1;comment:支付状态 1-待核验 2-已确认 3-待支付 4-已退款" json:"payment_status"`
	...
	PeopleCount      string             `gorm:"column:people_count;size:20;comment:拍摄人数(如 2大1小)" json:"people_count"`
	ShootStyle       string             `gorm:"column:shoot_style;size:50;comment:拍摄风格" json:"shoot_style"`
	SourceType       int                `gorm:"column:source_type;type:tinyint;default:1;comment:订单来源 1-管理端录入 2-客户预约 3-线索报价转化" json:"source_type"`
	PrepContent      string             `gorm:"column:prep_content;type:text;comment:拍前准备清单内容" json:"prep_content"`
	PrepReadAt       *string            `gorm:"column:prep_read_at;comment:客户确认阅读拍前准备时间" json:"prep_read_at"`
	SelectDeadline   *string            `gorm:"column:select_deadline;comment:选片截止时间(逾期默认全选)" json:"select_deadline"`
	DeliveryExpireAt *string            `gorm:"column:delivery_expire_at;comment:成片下载有效期至" json:"delivery_expire_at"`
	IsArchived       int                `gorm:"column:is_archived;type:tinyint;default:0;comment:是否已归档 0-否 1-是" json:"is_archived"`
	ArchivedAt       *string            `gorm:"column:archived_at;comment:归档时间" json:"archived_at"`
...
type OrderPayment struct {
	...
	Status       enum.PaymentStatus `gorm:"column:status;type:tinyint;default:1;comment:状态 1-待核验 2-已确认 3-待支付 4-已退款" json:"status"`
...
type OrderRefund struct {
	...
	ReasonLabel       string            `gorm:"column:reason_label;size:50;comment:取消原因分类" json:"reason_label"`
	ApplySource       int               `gorm:"column:apply_source;type:tinyint;default:1;comment:申请来源 1-管理端 2-客户H5/小程序" json:"apply_source"`
	RefundRule        string            `gorm:"column:refund_rule;size:50;comment:退款规则档位(按拍摄前小时)" json:"refund_rule"`
	Status            enum.RefundStatus `gorm:"column:status;type:tinyint;default:1;comment:状态 1-申请中 2-已通过 3-已退款 4-已驳回" json:"status"`
	...
	Voucher           string            `gorm:"column:voucher;size:500;comment:退款转账凭证截图" json:"voucher"`
	CustomerConfirmAt *string           `gorm:"column:customer_confirm_at;comment:客户确认收到退款时间" json:"customer_confirm_at"`
}
