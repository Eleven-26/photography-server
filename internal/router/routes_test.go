...
// 37（与 PC 同路径复用，含补开的 4 条 /notification/*、2 条取消/改期发起、1 条 /delivery/items/:id，
// 以及 2026-09-14 补开的 8 条：/customer/create、账号自助 /user/{profile,change-password,logout}、
// 收款方式 /settings/payment-method/{list,create,update,delete}）
// + 6（异路径别名）+ 2（真实端差异）+ 18（移动端独有）= 63。
// 有意增减员工端暴露面时须同步更新此基线——它防的是"悄悄多挂 / 漏挂"。
const staffRouteCountBaseline = 63
