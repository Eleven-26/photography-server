-- ============================================================================
-- 02-xxl-job-tables.sql —— XXL-JOB 调度中心的 MySQL 库表
--
-- 【谁在用】mysql 容器首次初始化时由 /docker-entrypoint-initdb.d/ 自动执行
--          （仅数据卷为空时跑一次，见 docker-compose.yml 的 mysql.volumes）
-- 【来源】xuxueli/xxl-job @ 3.4.2   doc/db/tables_xxl_job.sql
--         （与 .env 的 XXLJOB_ADMIN_VERSION 严格对应）
-- 【为什么需要】xxl-job-admin 首启即连 xxl_job 库并依赖其表结构，库表不存在则容器起不来。
--         官方脚本自带 `CREATE database if NOT EXISTS xxl_job` + `use xxl_job`，
--         故本文件不再重复添加建库语句。
-- ⚠️ 升级 xxl-job-admin 版本时须同步替换本文件。
-- ============================================================================
