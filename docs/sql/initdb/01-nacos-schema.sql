-- ============================================================================
-- 01-nacos-schema.sql —— Nacos 配置中心 / 注册中心的 MySQL 库表
--
-- 【谁在用】mysql 容器首次初始化时由 /docker-entrypoint-initdb.d/ 自动执行
--          （仅数据卷为空时跑一次，见 docker-compose.yml 的 mysql.volumes）
-- 【来源】alibaba/nacos @ 3.2.4（与 .env 的 NACOS_VERSION 严格对应，勿跨版本混用）
--         plugin-default-impl/nacos-default-datasource-plugin/
--         nacos-datasource-plugin-mysql/src/main/resources/META-INF/mysql-schema.sql
-- 【为什么需要】Nacos 3.x 起改用 MySQL 存储（原 standalone 内嵌 Derby 落在容器可写层，
--         容器一 recreate 配置与注册信息全丢）。官方镜像内已删除自带的 schema 文件，
--         必须由外部提供（官方示例 example/image/mysql/8/Dockerfile 即把同一份 schema
--         塞进 /docker-entrypoint-initdb.d/）。
-- 【表数量】13 张（config_info / config_info_gray / config_tags_relation / group_capacity
--         / his_config_info / tenant_capacity / tenant_info / users / roles / permissions
--         / pipeline_execution / ai_resource / ai_resource_version）
--
-- ⚠️ 原始 schema 不含 CREATE DATABASE / USE（官方靠 MYSQL_DATABASE 预建库），
--    本文件补上建库与选库两行。**这两行必须各自单行**：SQL 格式化器会把
--    `USE` 与库名拆成两行，导致整库初始化失败（同 docs/sql/README.md 对 ddl.sql 的警告）。
-- ⚠️ 升级 Nacos 版本时须同步替换本文件（官方 schema 会随版本增表/改列）。
-- ============================================================================
