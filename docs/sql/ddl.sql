...
--        a. id
--        b. company_id / store_id          —— 租户与门店键
--        c. code / name / title / username —— 业务标识
--        d. status / type / category / level 等 —— 主状态与分类
--        e. *_id                           —— 关联外键
--        f. *_status / *_type              —— 派生状态
--        g. 其余业务字段（金额、时间、内容等）
--        h. remark / note / description / sort / ext 等 —— 低权重辅助字段
--        i. 审计字段**固定沉底**（见下）
--   2) 审计字段固定放最后，顺序为：created_by 创建人 / created_at 创建时间 /
--      updated_by 修改人 / updated_at 修改时间 / deleted 是否删除(软删除)。
--      （唯一例外：sys_role_permission 采用全量覆盖式保存、不设软删除，
--        故只有 created_at，其仍置于最末。）
--   3) 每张业务表均含 company_id，用于 SaaS 多租户(一公司多门店、多门店多管理员)
--   4) 所有字段均带中文注释
-- =====================================================================
...
(
    ...
    `level`        tinyint        NOT NULL DEFAULT '1' COMMENT '客户等级 1-普通 2-黄金 3-铂金 4-钻石',
    ...
    `avatar`       varchar(500)            DEFAULT NULL COMMENT '头像地址',
    `allow_notifications` tinyint         NOT NULL DEFAULT '1' COMMENT '通知许可 0-不允许 1-允许',
    `prefer_style`        varchar(100)             DEFAULT NULL COMMENT '偏好风格(如 自然·生活感)',
    `prefer_scene`        varchar(100)             DEFAULT NULL COMMENT '常用场景(如 户外公园)',
...
(
    ...
    `photographer_id` bigint   NOT NULL DEFAULT '0' COMMENT '指定摄影师ID(0=未指定，由门店/工作室认领)',
    `photographer`    varchar(50)       DEFAULT NULL COMMENT '指定摄影师姓名(快照)',
    ...
    KEY               `idx_custom_req_photographer` (`photographer_id`),
