...
> **当前阶段：项目未上线**（`ddl.sql` + `dml.sql` 即最新结构 / 最新数据）。
>
> ⚠️ **结构变更一律「先写增量、再合并全量」**（2026-09-15 起生效，不再只改全量）：
> `增量/ddl-初版.sql` / `dml-初版.sql` 是增量链起点的**第一版快照，已冻结**（不再随全量同步）；
> 每次变更在 `增量/` 新增一个 `upgrade_*.sql`，再把**同样的改动**合并进 `ddl.sql` / `dml.sql`。
>
> 这样「全量」与「增量链」始终等价：新库用全量一次初始化，老库按增量链逐次升级。
...
每积累一个 `upgrade_*.sql` 就按下列方式执行（未上线期间同样适用，例如本地 / 测试库）：
...
## 三、维护流程（写增量 + 合并全量 + 验证一致）

**结构或数据变更一律走这三步**（2026-09-15 起，未上线期间同样如此）：

1. **写增量** — 在 `docs/sql/增量/` 新增 `upgrade_变更描述_YYYYMMDD.sql`，只写本次变更
   （`ALTER TABLE` / `CREATE TABLE` / `INSERT` / `UPDATE`）。**初版文件已冻结，此后不再改动。**
...
> **排版**：`ddl.sql` 的列定义按「字段名 → 类型 → 约束 → 中文注释」对齐，
> 且每张表的字段名列宽按该表最长字段名统一补位。新增字段后如列宽变化，需整表重新对齐。

### 命名规范

- 增量文件：`upgrade_变更描述_YYYYMMDD.sql`（如 `upgrade_studio_setting_cover_url_20260915.sql`）
...
> **每次改动都要跑**：它把 `增量/ddl-初版.sql` + `dml-初版.sql` + 各 `upgrade_*.sql`（按日期升序）
> 依次执行成一个临时库，再与「`ddl.sql` + `dml.sql`」对拍。
> **写了增量却忘了合并全量（或反之），会立刻报不一致** —— 这正是它的价值。
>
...
- **⚠️ 初版（`增量/ddl-初版.sql` / `dml-初版.sql`）已冻结，不要再往里改内容**：它是增量链的**起点快照**。
  现在的对应关系是「**初版 + 各 `upgrade_*.sql` == 全量**」，而不是「初版 == 全量」。
  往初版里直接改内容会与增量脚本**重复施加同一变更**（如重复加列），`verify_consistency.sh` 必报不一致。
